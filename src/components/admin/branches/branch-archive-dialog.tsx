...
export function BranchArchiveDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.message || 'Failed to archive branch. Please check active classroom/batch dependencies.');
    }
  };
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                Are you sure you want to archive <strong className="text-slate-700 dark:text-slate-250">{branchName}</strong>? 
                This will soft-delete the record and remove it from active options.
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              ...
              <textarea
                ...
                rows={3}
                required
                placeholder="e.g. Relocating office or closing training center due to lease expiry"
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                className="w-full px-3 py-2 bg-slate-50/50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-lg text-slate-950 dark:text-white placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 transition text-sm"
              />
              {error && (
...
export default BranchArchiveDialog;
