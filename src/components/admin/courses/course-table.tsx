...
import { Edit2, MoreVertical, Archive, BookOpen, CheckCircle2, XCircle, Star, GraduationCap } from 'lucide-react';
import { CourseStatus } from '@prisma/client';
import { publishCourseAction, unpublishCourseAction, archiveCourseAction, restoreCourseAction } from '@/modules/courses/course.actions';
...
export function CourseTable({ courses, userPermissions, onRefresh }: CourseTableProps) {
  const [loadingId, setLoadingId] = useState<string | null>(null);

  const canEdit = userPermissions.includes('course.edit');
  const canPublish = userPermissions.includes('course.publish');
  const canArchive = userPermissions.includes('course.archive');
  const canRestore = userPermissions.includes('course.restore');
    ...
    return (
      <div className="bg-white dark:bg-slate-900 border border-slate-205 dark:border-slate-800 rounded-xl p-12 text-center">
        ...
        <p className="text-slate-500 mt-1 max-w-sm mx-auto">
          Get started by creating your first course within a vertical.
        </p>
      </div>
    );
  }
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead className="bg-slate-50 dark:bg-slate-950/50 border-b border-slate-205 dark:border-slate-800 text-slate-500 dark:text-slate-400">
            <tr>
              ...
              <th className="px-6 py-4 font-medium text-center">Modules</th>
            ...
            {courses.map((course) => (
              <tr 
                ...
                className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors ${
                  course.status === CourseStatus.ARCHIVED ? 'opacity-60 grayscale' : ''
                }`}
              >
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    {canPublish && course.status !== CourseStatus.PUBLISHED && course.status !== CourseStatus.ARCHIVED && (
                      <button
                        disabled={loadingId === course.id}
                        onClick={() => handleAction(course.id, course.recordVersion, publishCourseAction)}
                    ...
                    {canPublish && course.status === CourseStatus.PUBLISHED && (
                      <button
                        disabled={loadingId === course.id}
                        onClick={() => handleAction(course.id, course.recordVersion, unpublishCourseAction)}
                    ...
                    {canArchive && course.status !== CourseStatus.ARCHIVED && (
                      <button
                        disabled={loadingId === course.id}
                        onClick={() => handleAction(course.id, course.recordVersion, archiveCourseAction)}
                    ...
                    {canRestore && course.status === CourseStatus.ARCHIVED && (
                      <button
                        disabled={loadingId === course.id}
                        onClick={() => handleAction(course.id, course.recordVersion, restoreCourseAction)}
                        className="p-2 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition"
                        title="Restore"
                      >
